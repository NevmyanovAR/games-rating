...
const { config, calculations } = require("../appModules/rating");
...
async function voteRouteController(req, res) {
  if (req.method !== "POST") {
    ...
    try {
      ...
      const rating = calculations.createRating(data, config.WEIGHT);

      const ratingFile = await fs.readFile(config.PATH_TO_RATING_FILE);
      const ratingArray = JSON.parse(ratingFile);
      const newRating = calculations.updateRating(ratingArray, data.id, rating);
